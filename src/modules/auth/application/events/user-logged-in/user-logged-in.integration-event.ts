 ...
 *
 * Integration event published when a user logs in, for external systems (audit, analytics, etc).
 * This event is stable and decoupled from the internal domain event.
 */
export class UserLoggedInIntegrationEvent implements IntegrationEvent {
  ...
  /** Event type (optional) */
  readonly eventType?: string = 'UserLoggedInIntegrationEvent';
  ...
  /**
   * Creates a new UserLoggedInIntegrationEvent
  ...
  constructor(params: {
    ...
    this.eventType = params.eventType ?? 'UserLoggedInIntegrationEvent';
